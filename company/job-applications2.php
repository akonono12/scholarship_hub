            ...
            <div class="box box-solid">
              ...
              <div class="box-body no-padding">
                <ul class="nav nav-pills nav-stacked">
                  ...
                  <li class="active"><a href="job-applications.php"><i class="fa fa-file-o"></i> Scholarship Application</a></li>
              ...
              <div class="col-md-12">
                <div class="box-body table-responsive no-padding">
				  ...
				  $da=base64_decode(urldecode($_GET['a']));
					$db=base64_decode(urldecode($_GET['b']));
					$dc=base64_decode(urldecode($_GET['c']));
					$dd=base64_decode(urldecode($_GET['d']));
					   $ssstt=$conn->query("SELECT  * FROM `apply_scholar` inner JOIN post_scholar on apply_scholar.scholar_id=post_scholar.scholar_id inner join applicant on applicant.Applicant_id=apply_scholar.Applicant_id inner join application_form on apply_scholar.Application_id=application_form.Applictn_id where post_scholar.agency_id = '$_SESSION[id_company]' AND apply_scholar.scholar_id = '$da' and apply_scholar.status <> 3 and apply_scholar.status <> 1 and apply_scholar.tl_pts between '80' and '100' ORDER BY apply_scholar.`tl_pts` DESC,application_form.Fam_grinc ASC  limit $dd");
					 
					     if($ssstt->num_rows > 0){
					   while( $rowss=$ssstt->fetch_assoc()){  
					
					 $familyinc=$rowss['Fam_grinc'] ;
					   }
					   }
                       $sql = "SELECT  *,IF(apply_scholar.tl_pts >90 and application_form.Fam_grinc <='$familyinc' , 'Yes', 'No') as qualified FROM `apply_scholar` inner JOIN post_scholar on apply_scholar.scholar_id=post_scholar.scholar_id inner join applicant on applicant.Applicant_id=apply_scholar.Applicant_id INNER join application_form on apply_scholar.Application_id=application_form.Applictn_id where post_scholar.agency_id = '$_SESSION[id_company]' AND apply_scholar.scholar_id = '$da' and apply_scholar.status <> '1' ORDER BY `apply_scholar`.`AppSc_id` ASC ";
                      ...
                      <tr>
                        ...
                        <td><a href="user-application.php?appid=<?php echo $row['Applicant_id'];?>&idb=<?php echo $da;?>&opid=1" > <span class="glyphicon glyphicon-folder-open"></span></a></td>
                      </tr>
