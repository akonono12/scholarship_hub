...
//prevent uploading of wrong file types 
    ...
    window.location.href='application_form2.php?id=".$_GET['id']."';
    </script>");
	}else{
		$continue = 1;
...
$connect->query($insApp);
 ...
 for($count = 0; $count < count($_POST["item_name"]); $count++){  
  ...
  (app_id,scholar_id,Subject_name,  Subject_Grade) 
  VALUES ( :app_id, :scholar_id, :subjname,  :grade)
  ";
...
$selsubj=$connect->query("SELECT sum(grades.Subject_Grade) as totalsum, count(*) as totalcount FROM `grades` WHERE grades.app_id = '$_SESSION[id_user]' and scholar_id ='$_GET[id]' ");

$rows= $selsubj->fetch(); 

 $gwa=$rows['totalsum']/$rows['totalcount'];
