...
if(isset($_GET['filter']) && $_GET['filter']=='city') {
  ...
  if(isset($_GET['filter']) && $_GET['filter']=='searchBar') {
    ...
    $search = $_GET['search'];
    $sql = "SELECT * from post_scholar inner join agency on post_scholar.agency_id=agency.Agency_id where agency.Agency_Name like '$search%' and post_scholar.Status  = '0' LIMIT $start_from, $limit";
  ...
  if($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
         ...
         <div class="attachment-block clearfix">
              ...
              <div class="attachment-pushed">
                <h4 class="attachment-heading"><a href="view-job-post.php?id=<?php echo $row['scholar_id']; ?>">View</a> <span class="attachment-heading pull-right"><?php echo $row['Quota_applicant']; ?> Available Slot(s)</span></h4>
                <div class="attachment-text">
                    <div><strong><?php echo $row['Agency_Name']; ?> | <?php echo $row['Agency_Address']; ?> | Deadline: <?php echo $row['deadline']; ?></strong></div>
                  </div>
                </div>
              </div>
